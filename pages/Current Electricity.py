...
4. Vary the voltage using the power supply or rheostat and repeat the measurements for several values of \( V \) and \( I \).
5. Calculate the resistance \( R \) for each pair of readings using the formula \( R = \frac{V}{I} \).
...
with st.expander("**1. Question:** In an experiment to verify Ohm’s Law, a student sets up a circuit with a resistor of"
                 " \( 10 \, \Omega \). The student measures a current of \( 0.5 \, \text{A} \) when the applied voltage"
                 " is \( 5 \, \text{V} \). Does this confirm Ohms Law? Show the calculation."):
    ...
    st.info("""**Answer:**
            ...
            The calculated resistance matches the resistor's given value (\( 10 \, \Omega \)). Thus, the results 
            confirm Ohm's Law.
            """)
...
with st.expander("""**2. Question:**
    ...
    st.info("""**Answer:**The **voltmeter** measures the potential difference across the resistor. It must be connected
            in **parallel** so it measures the voltage drop across the resistor without affecting the current flow through it.
            Voltmeters have high resistance, minimizing current flow through them.
            - The **ammeter** measures the total current flowing in the circuit. It must be connected in **series** to ensure the
            same current flows through both the ammeter and the resistor. Ammeters have very low resistance to avoid altering
...
with st.expander("""**3. Question:**
    ...
    st.info("""**Answer:**
            ...
            1. **Non-ohmic behavior:** The resistor may not be an ohmic conductor. Examples include diodes or filament bulbs,
             where resistance changes with temperature or voltage.
            2. **Overheating of the resistor:** At higher currents, the resistor might heat up, changing its resistance.
            3. **Faulty apparatus:** Errors in the connections, or a malfunctioning resistor, voltmeter, or ammeter, can cause
             deviations.
