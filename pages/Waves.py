...
st.warning("Suggestion: Brush some concepts of Gravitation before proceeding as some Qs on that topic are included.")
...
2. The gravitational force exerted by a spherically symmetric body of mass m1 on another such body of mass m2 kept 
outside the first body is Gm1m2r^A3, where r is the distance between the centers of the two bodies."""
...
"""
3. Water falling from a 50 m high fall is used for generating electrical energy. If 1.8 * 10^5 kg of water falls per
...
"""
5. Velocity of a travelling wave on a string ‘v’ is related to the tension T and the linear mass density ‘m’ as: 
v^2 = T * m^A4.
"""
...
"""
6. Equation of a transverse wave travelling in a rope is given by y = 4 sin (5.0 t - 0.01 x), where y and x are 
...
7. A fluid has bulk modulus of 4 * 10^9 Pa and density 10^3 kg/m3. The speed of sound in that fluid is 10 * (A10) m/s.
"""
...
"""
8. By Newton`s Formula the speed of sound at one atmospheric pressure is nearly 280 m/s. CP/CV for air is 7/4. The 
actual speed of sound in air is nearly A13 m/s.

9. The amount of energy which flows perpendicular to a plane per unit area per unit time in a wave is proportional to 
...
3. A wave is travelling along the X-axis. The disturbance at x = 0 and t = 0 is A/2 and is increasing, where A is 
amplitude of the wave. If y = A sin (kx - wt + φ), the initial phase φ = π / (A9 + A14). True /False.

4. A long string of mass per unit length 0.01 kg/m is subject to a tension of 64 N along the x-axis. One end (x=0) of 
...
5. A travelling wave is given by  Y = 0.8 / (3x^2 + 24xt + 48t^2 + 4)  where x and y are in metres and t is in second. 
The wave speed is (A3)^2. True / False.

6. A sky-diver falls with a terminal speed B3 m/s carrying a buzzer emitting a steady tone at 1800 Hz. The speed of 
...
5. The fundamental frequency of a sono-meter wire increases by 6 Hz if the tension is increased by 44 %, keeping the 
length constant. The change in fundamental frequency of the sonometer is | A5 + A6 + A7 | hz when the length of wire is 
increased by 20 %, keeping the original tension in the wire constant. True / False.

6. The length of a pipe is l = 85 cm. The speed of sound is 340 m/s. When the pipe is open at both ends the 2nd harmonic
 frequency is 2 * A10 Hz and the third harmonic frequency is 3 * A12 Hz. True / False.

7. In the previous question, the sqrt (B4)th harmonic is C4 * 100 hz in an open pipe. The sqrt (B4)th harmonic does not 
exist for a closed pipe. True / False.

8. An open pipe 40 cm long and a closed pipe 31 cm long, both having the same diameter, are producing their first 
